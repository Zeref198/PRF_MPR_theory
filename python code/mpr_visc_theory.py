import os
import numpy as np
from numpy import pi, sinh, cosh, tanh, sqrt
...
def T_func(rho1, rho2, sig1, sig2, Lx, Ly, h1, h2, k1, k2):
    """
    ...
    if k1 != k2:
        ...
        fac1 = Lambda1 / (k1 * (k1**2 - k2**2) * Lx * Ly)
        term1 = (
            k1 / tanh(k2 * h2)
            - k2 / tanh(k1 * h2)
        ...
        fac2 = Lambda2 / (k2 * (k2**2 - k1**2) * Lx * Ly)
        term2 = (
            k2 / tanh(k1 * h2)
            - k1 / tanh(k2 * h2)
        ...
        T_norm = (Lambda1 / (4 * pow(k1, 2) * pow(np.sinh(k1 * h2), 2) * Lx * Ly)) * (
...
def growth_rate(rho1, rho2, nu1, nu2, sig1, sig2, gamma,
    ...
    T1, T2 = T_func(rho1, rho2, sig1, sig2, Lx, Ly, h1, h2, k1, k2)

    U1 = (gamma * k1**2 + (rho2 - rho1) * g)
    U2 = (gamma * k2**2 + (rho2 - rho1) * g)
...
# parameter check
def parameter_check(electrolyte, metal, surface_tension, geometry, wavemode1=None, wavemode2=None):
    """
    ...
    if (wavemode1 is not None) and (wavemode2 is not None):
...
def calculate_viscous_damping(
    ...
    """
    Compute the viscous damping as the sum of the contributions.
    """
...
def calculate_SeleParameter(
    ...
    """
    Compute the modified Sele parameter. Definition of β in equation (41).
    """
    # initiate variables
    rho1 = electrolyte.density
    rho2 = metal.density
...
def calculate_instability_onset(
    ...
    """
    Compute the instability onset based on the more unstable wavenumber.
    """
    rho1 = electrolyte.density
    rho2 = metal.density
    nu1 = electrolyte.kinematic_viscosity
