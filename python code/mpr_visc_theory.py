...
def visc_damp_wall(rho1, rho2, nu1, nu2, gamma, m, n, Lx, Ly, h1, h2):
    ...
    for i in ([1,2]):
        ...
        common_factor = pow(2,-0.5) * ( rho*sqrt(w*nu)/(k*Lx*Ly) )/(
            (rho1*coth(k*h1) + rho2*coth(k*h2)) )
        
        base_faces = Lx*Ly*pow(k,2)/(2*pow(sinh(k*h), 2) )
        sidewall = (
                    deltaFunc(m)*h*(pow(n*pi,2) - pow(k*Ly,2))/(2*Ly*pow(sinh(k*h),2))
                    +
                    deltaFunc(m)*(pow(n*pi,2) + pow(k*Ly,2))/(2*Ly*k*tanh(k*h))
                    +
                    deltaFunc(n)*h*(pow(m*pi,2) - pow(k*Lx,2))/(2*Lx*pow(sinh(k*h),2))
                    +
                    deltaFunc(n)*(pow(m*pi,2) + pow(k*Lx,2))/(2*Lx*k*tanh(k*h))
                   )
...
def T_func(rho1, rho2, sig1, sig2, h1, h2, k1, k2):
    ...
    if k1 != k2:
        ...
        term1 = (
                k1/tanh(k2*h2) - k2/tanh(k1*h2) 
                + k2/(sinh(k1*h1)*cosh(k2*h1))
                - k2/tanh(k1*h1) + k1*tanh(k2*h1)
                )
        T_norm = fac1*term1
        
        fac2 = Lambda2/(k2*(k2**2 - k1**2))
        term2 = (
                k2/tanh(k1*h2) - k1/tanh(k2*h2) 
                + k1/(sinh(k2*h1)*cosh(k1*h1))
                - k1/tanh(k2*h1) + k2*tanh(k1*h1)
                )
        T_dash = fac2*term2
    
    else:
        T_norm = (
                  ( Lambda1/ (4*pow(k1,2)*pow(np.sinh(k1*h2),2)) ) 
                  * 
                  ( np.sinh(k1*h1+2*k1*h2)/np.cosh(k1*h1) + 2*k1*h2 - np.tanh(k1*h1) )
                 )
        T_dash = T_norm
...
def parameter_check(electrolyte, metal, geometry):
    ...
    sig1=electrolyte.electric_conductivity; sig2=metal.electric_conductivity
...
def calculate_GrowthRate(electrolyte, metal, 
                        surface_tension, 
                        geometry, power,
                        wavemode1, wavemode2):
    ...
    gr = growth_rate(rho1, rho2, nu1, nu2, sig1, sig2, 
                     gamma, m1, n1, m2, n2, 
                     Lx, Ly, h1, h2, 
                     I0, Bz, damp1, damp2)
...
def calculate_instability_onset(electrolyte, metal, 
                                surface_tension, 
                                geometry, 
                                wavenumber_limit):
    ...
    def genWaveNo(wavenumber_limit):
        ...
        while i < len(comb_set):
            ...
            cond4 = (np.cos(m1*pi)*np.cos(m2*pi)-1)*(np.cos(n1*pi)*np.cos(n2*pi)-1)
    ...
    for j in range(len(comb)):
        ...
        beta = betaCrit_func(rho1, rho2, nu1, nu2, sig1, sig2, 
                             gamma, m1, n1, m2, n2, Lx, Ly, h1, h2, damp1, damp2)
