...
\end{equation}
%%%%%%%%%%%%%%%%%%%%
Here, $\widehat{\h u}_i, \widehat p_i \widehat\eta$ are the time-independent form of $\h u_i, p_i, \eta$. The flow potentials are now given by
%%%%%%%%%%%%%%%%%%%%
\begin{equation}
	\begin{bmatrix}
...
\end{equation} 
%%%%%%%%%%%%%%%
Here, $A_{mn}$ is a mode-dependent wave amplitude, $k$ is a function of the wave numbers $(m,n)$
%%%%%%%%%%%%%%%
\begin{equation}
...
%%%%%%%%%%%%%%%%%%%%
\\
\indent The wave perturbs the interface ans causes deviation of the electrical current. We calculate the current perturbations through Ohm's law $\widehat{\h j}_i = -\sigma_i\bm\nabla \widehat\varphi_i$. The electric potentials must also fulfill the Laplace equation $\bm{\nabla}^2\widehat{\varphi} = 0$ with boundary conditions given in Eqs.\,(\ref{eq:bc1} -- \ref{eq:bc4}), \eqref{eq:int_bc3} and \eqref{eq:int_bc4}. We find the solutions to the electrical potential to be
%%%%%%%%%%%%%%%%%%%%
\begin{equation}
	\begin{bmatrix}
...
\begin{figure}
	...
	\end{tikzpicture}
	\caption{An illustration of a linear gravity-capillary wave in a rectangular system of lateral aspect ratio equal to 2. For example, the superposition of a two standing waves of modes $(2,0)$ and $(1,1)$ to produce of a rotating interfacial wave of mode $(2,0)+(1,1)$.}
...
Up to this point, we have been looking at the MPR instability at the inviscid limit. In reality, viscosity plays a crucial role in dissipating some of the energy injected in the system. To evaluate the stability of industrial ARCs, in which turbulent flows usually prevail, the damping rate due to viscous dissipation must be estimated or determined experimentally. In small-scale models, where flows remain largely laminar (at least in the boundaries or around the point of marginal instability), viscous damping rates can be calculated analytically. Here, viscosity is treated as a perturbation independent of the Lorentz force and the damping rate is subtracted from the inviscid growth rate in the two-wave ansatz [Eq.\,\eqref{eq:perturb_method_ansatz}] and solved accordingly. This is a key ingredient for our model to serve as a benchmark. 
\\
