...
\end{equation}
%%%%%%%%%%%%%%%
The product $\mathcal{T}\mathcal{T'}$ from Eqs.\,\eqref{eq:T_11} and \eqref{eq:T_22}, simplifies to
\begin{equation}
	\mathcal{T}\mathcal{T'} = \frac{1}{k^4k'^4h_1^2h_2^2},
\end{equation}
and therewith Eq.\,\eqref{eq:beta_crit-onset_visc} reduces to
%%%%%%%%%%%%%%%
\begin{equation}
...
This chapter is dedicated to the validation of our model, which essentially consists of the two-wave MPR growth rate [Eq.\,\eqref{eq:main_gr_visc}] and corresponding instability thresholds [Eq.\,\eqref{eq:beta_crit-onset_visc}] combined with the independently derived viscous [Eq.\,\eqref{eq:visc_damp_sum}] damping rate. To this end we compare several predictions with different previous studies, whereby we also want to scrutinize some interesting aspects of the MPR physics deducible from our model.
...
For a simple explanation, let us first consider the function $\Theta$ [Eq.\,\eqref{eq:form_function}], which can only be non-zero if both wave numbers $(m,m')$ and $(n,n')$ are of opposite parity. Wave pairs whose wave numbers $(m,m')$ or $(n,n')$  are both even or odd can therefore never be destabilized. From number theory, all sums and differences
...
\end{equation}
%%%%%%%%%%%%%%%
By inserting the most unstable wave numbers [Eq.\,\eqref{eq:mn_odd}] into the growth rate solution in Eq.\,\eqref{eq:main_gr} and applying all the mentioned limits we obtain an expression for $\alpha$ as a function of the fractal numbers
\begin{equation}
