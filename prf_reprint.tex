...
\begin{equation}
	...
	\begin{bmatrix}
		\bm{\nabla}{\phi}_i \\[5pt]
		-\rho_i(\ii i\omega){\phi}_i\\[5pt]
		\partial_z {\phi}_i/(\ii i\omega )
	\end{bmatrix}.
	\label{eq:pot_wave_soln}
\end{equation}
%%%%%%%%%%%%%%%%%%%%
Here, the flow potential in both layers are given by
%%%%%%%%%%%%%%%%%%%%
\begin{equation}
	\begin{bmatrix}
		{\phi}_{1} \\[12pt]
...
\end{equation} 
%%%%%%%%%%%%%%%
Here, $A_{mn}$ is a mode-dependent wave amplitude and $k$, a function of the wave numbers $(m,n)$ that count the number of half wavelengths along $x$ and $y$ is 
%%%%%%%%%%%%%%%
\begin{equation}
...
\end{equation}
%%%%%%%%%%%%%%%%%%%%
In this notation, the tilded variables are small perturbations with respect to the hatted variables representing the time-independent form of the basic two-wave state. The bare and the dashed($'$) hatted variables denote the variables corresponding to the two wave number ($m,n$) and $(m',n')$ accordingly. The terms $C$ and $C'$ are to be determined and $\alpha$ is the potentially destabilizing contribution to the inviscid growth rate that we seek to derive. We assume that both waves are magnetohydrodynamically coupled and therefore, both oscillate with the same mean frequency. We represent the average natural frequency of the wave as $\bar{\omega} = (\omega + \omega')/2$ and the average frequency difference as $\delta\omega = (\omega - \omega')/2$.  Considering the wave pair to be in a coupled motion $\omega = \omega' = \bar{\omega}$ is justified by the fact that the wave frequencies are shifted by Lorentz forces ($\bar{\omega} = \omega - \delta \omega$ and $\bar{\omega} = \omega' + \delta \omega$) in such a way that they coincide exactly at the point of marginal stability \cite{Davidson1998}. In the absence of any Lorentz forces, precisely the classic gravity-capillary wave numbers given in Eq.\,(\ref{eq:perturb_method_ansatz}) result as two independent solutions of our system. This rigorously justifies our approach, and our theory is capable of describing the complete transition from purely hydrodynamic waves passing through magnetohydrodynamically modified waves regimes up to the (linear) self-amplifying MPR wave.
\\
\indent All the preparations have now been made to calculate the inviscid growth rate $\alpha$. By inserting the ansatz [Eq.\,\eqref{eq:perturb_method_ansatz}] in the governing Eqs.\,\eqref{eq:gov_momentum} and \eqref{eq:gov_cont}, and considering only the leading order terms, we can obtain the next order in the perturbation problem through the equations:
%%%%%%%%%%%%%%%%%%%%
\begin{subequations}
	\begin{align}
		\nn
...
\end{subequations}
%%%%%%%%%%%%%%%%%%%%
The terms given above are associated with the power transfers between waves mediated by Lorentz forces. The terms $K_{\eta\eta}$ is associated with the potential (or equivalently kinetic) energy of the respective wave number $(m,n)$ and is given by
%%%%%%%%%%%%%%%%%%%%
\begin{align}
	\nn
...
%%%%%%%%%%
\begin{equation}
	\alpha = \pm \; \sqrt{\dfrac{I_{\eta\eta'}I_{\eta'\!\eta}}{4K_{\eta\eta}K_{\eta'\!\eta'}} + \bigl[ \ii i{\delta \omega} \bigr]^2 }.
\end{equation}
%%%%%%%%%%%%%%%%%%%%
